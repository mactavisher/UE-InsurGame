...
protected:
	...
	/** send a initial replication to relevant clients for extremely fast moving projectiles,work with InitRepTickFunc */
...
public:
	...
	virtual float GetBaseDamage() const { return DamageBase; }

	virtual void SetScanTraceHitLoc(const FVector NewLoc) { ScanTraceHitLoc = NewLoc; }

	/** get the current penetrate count */
	virtual uint8 GetCurrentPenetrateCount() const { return CurrentPenetrateCount; }
	...
	virtual void SetInstigatedPlayer(class AController* InstigatedPlayer) { InstigatorPlayer = InstigatedPlayer; }

	/** set is fake projectile */
	virtual void SetIsFakeProjectile(const bool bFake) { this->bVisualProjectile = bFake; }
	...
	virtual bool GetIsFakeProjectile() const { return bVisualProjectile; }
	 ...
	 */
	virtual void SetMuzzleSpeed(const float NewSpeed);
	 ...
	 */
	virtual void SetScanTraceProjectile(const bool bFromScanTrace) { bScanTraceProjectile = bFromScanTrace; }
	 ...
	 */
	virtual void SetClientFakeProjectile(class AINSProjectile* const NewFakeProjectile) { VisualFakeProjectile = NewFakeProjectile; };
};
